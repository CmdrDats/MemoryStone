...
public class Interference {
    ...
    public void update(Player player, PlayerMoveEvent event) {
        ...
        for (MemoryStone stone : MemoryStonePlugin.getInstance().getMemoryStoneManager().getStones()) {
            if (stone.getSign() == null || stone.isGlobal()
                    || !stone.getSign().getWorld().getName().equals(player.getWorld().getName())
                    || MemoryStonePlugin.getInstance().getCompassManager().isMemorized(player, stone)) {
                continue;
            }
